...
pub fn init_device<R: Runtime>(app: &AppHandle<R>) {
    ...
    let mut left_eye_image_msg_rx = serial.get_message_rx();
    ...
    let mut left_eye_image_stream_response_rx = left_eye_image_stream.get_response_rx();
    std::thread::spawn(move || {
        left_eye_image_stream.start();
    });
    // init right eye image stream
    let mut right_eye_image_msg_rx = serial.get_message_rx();
    ...
    let mut right_eye_image_stream_response_rx = right_eye_image_stream.get_response_rx();
    std::thread::spawn(move || {
